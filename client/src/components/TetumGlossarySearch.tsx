    ...
    return (
      <Card className="absolute top-full left-0 right-0 z-50 mt-1 border-gray-200 shadow-lg bg-white">
        <CardContent className="p-4 text-center text-gray-500 text-sm">
          No Tetum glossary terms found for "{searchTerm}"
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="absolute top-full left-0 right-0 z-50 mt-1 max-h-64 overflow-auto border-gray-200 shadow-lg bg-white">
      <CardContent className="p-0">
        {filteredEntries.map((entry, index) => (
          ...
          >
            <div className="font-medium text-gray-900">{entry.tetum}</div>
  ...
  return (
    <div className="p-6">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Tetum Glossary</h2>
        <p className="text-gray-600">Legal Terms in Tetum with Detailed Explanations</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 relative" ref={searchRef}>
            <Input
              placeholder="Enter Tetum legal term (min 2 characters)"
              value={wordSearch}
      ...
      <div className="flex justify-center mb-8">
        <Button
          ...
          className="bg-green-500 hover:bg-green-600 text-white px-8 py-2 rounded-lg disabled:bg-gray-300 disabled:cursor-not-allowed"
          disabled={!wordSearch.trim() || wordSearch.length < 2}
        >
          <Search className="w-4 h-4 mr-2" />
          Search Tetum Glossary
        </Button>
      </div>
            ...
            <div className="mb-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => speakText(selectedEntry.tetum || "")}
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => copyToClipboard(selectedEntry.tetum || "")}
                  className="p-1 hover:bg-gray-100"
                >
                  <Copy className="h-4 w-4 text-gray-600" />
                </Button>
              </div>
              <p className="text-gray-600 text-sm mb-4">Legal term with detailed explanation in Tetum</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              {selectedEntry.explanation && (
                <div className="bg-green-50 rounded-lg p-4">
                  <h4 className="text-sm font-semibold text-green-800 mb-2">Explanation</h4>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-600">
                  <strong>Legal Context:</strong> This term is used in Timor-Leste's legal system 
                  and is sourced from official legal documentation and glossaries.
                </p>
              </div>
            </div>
          </div>
      ...
      {showResults && searchResults.length === 0 && !isLoading && (
        <div className="bg-white rounded-lg border border-gray-200 p-6 text-center">
          <p className="text-gray-500">No Tetum glossary terms found for "{wordSearch}"</p>
        </div>
      )}
      ...
      {isLoading && showResults && (
        <div className="bg-white rounded-lg border border-gray-200 p-6 text-center">
          <p className="text-gray-500">Searching Tetum glossary...</p>
        </div>
      )}
      ...
      {error && showResults && (
        <div className="bg-red-50 rounded-lg border border-red-200 p-6 text-center">
          <p className="text-red-600">Error searching Tetum glossary. Please try again.</p>
        </div>
      )}
    </div>
  );
